...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    adjusted_bb_frame.Translate(ImVec2(x_offset, 0.0f));
    adjusted_bb_frame.Min.y += y_offset;
    adjusted_bb_frame.Max.y -= y_offset;
    ...
    vec4 bgCol = {
        style.Colors[ImGuiCol_ScrollbarBg].x,
    ...
    window->DrawList->AddRectFilled(
        adjusted_bb_frame.Min + ImVec2(3, 3),
        adjusted_bb_frame.Max - ImVec2(3, 3),
        GetColorU32(bgCol),
        style.ScrollbarRounding
    );

    ImVec4 col = IsItemActive() ? 
        vec4(
            style.Colors[ImGuiCol_ScrollbarGrabActive].x,
            style.Colors[ImGuiCol_ScrollbarGrabActive].y, 
            style.Colors[ImGuiCol_ScrollbarGrabActive].z, 
            style.Alpha
        ) : IsItemHovered() ? vec4(
            style.Colors[ImGuiCol_ScrollbarGrabHovered].x, 
            style.Colors[ImGuiCol_ScrollbarGrabHovered].y,
            style.Colors[ImGuiCol_ScrollbarGrabHovered].z,
            style.Alpha
        ) : vec4(
            style.Colors[ImGuiCol_ScrollbarGrab].x,
            style.Colors[ImGuiCol_ScrollbarGrab].y,
            style.Colors[ImGuiCol_ScrollbarGrab].z,
            style.Alpha
        );

    static std::map<ImGuiID, __Scrollbar__> anim;
    __Scrollbar__& w = anim.emplace(id, __Scrollbar__(col)).first->second;

    w.time = gui->vtime(gui->GlobalSpeed);
    w.grabcol = ImLerp(w.grabcol, col, w.time);
    w.norm_pos = ImLerp(w.norm_pos, grab_v_norm, w.time);

    ImRect animated_grab_rect;
    if (axis == ImGuiAxis_X)
        animated_grab_rect = ImRect(
            ImLerp(bb.Min.x, bb.Max.x, w.norm_pos),
            bb.Min.y,
            ImLerp(bb.Min.x, bb.Max.x, w.norm_pos) + grab_h_pixels,
            bb.Max.y
        );
    else
        animated_grab_rect = ImRect(
            bb.Min.x,
            ImLerp(bb.Min.y, bb.Max.y, w.norm_pos),
            bb.Max.x,
            ImLerp(bb.Min.y, bb.Max.y, w.norm_pos) + grab_h_pixels
        );

    window->DrawList->AddRectFilled(
        animated_grab_rect.Min,
        animated_grab_rect.Max,
        GetColorU32(w.grabcol),
        style.ScrollbarRounding
    );
...
void ImGui::ImRotateEnd(float rad, ImVec2 center)
{
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        PushFont(fonts->Pass);
        const ImFontGlyph* glyph = g.Font->FindGlyph('e');
        PopFont();
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            static std::map <ImGuiID, InputAnim> anim;
            auto& w = anim.emplace(id, InputAnim{  }).first->second;

            float offset = frame_bb.Min.x + w.v - cursor_screen_rect.Min.x;
            if (offset != 0) {
                float direction = (offset > 0) ? -1.0f : 1.0f;
                w.b = std::abs(offset) / 12.f;
                w.v += direction * w.b / ImGui::GetIO().Framerate * 200.f;
            }

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddRectFilled(
                    frame_bb.Min + ImVec2(w.v + 0, 15),
                    frame_bb.Min + ImVec2(w.v + 1, 29),
